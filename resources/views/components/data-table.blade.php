...
@once
    @push('scripts')
        <script>
            function dtSelectAll(tableId, masterCheckbox) {
            ...
            function dtUpdateCount(tableId) {
                const table = document.getElementById(tableId);
                const totalCheckboxes = table.querySelectorAll('tbody input[type="checkbox"]').length;
                const checkedCheckboxes = table.querySelectorAll('tbody input[type="checkbox"]:checked').length;

                const masterCheckbox = document.getElementById('checkAll-' + tableId);
                if (masterCheckbox) {
                    masterCheckbox.checked = (totalCheckboxes > 0 && totalCheckboxes === checkedCheckboxes);
                }

                const el = document.getElementById('count-' + tableId);
                if (el) {
                    el.textContent = checkedCheckboxes + 'টি selected';
                    el.style.display = checkedCheckboxes > 0 ? 'inline' : 'none';
                }
            }

            document.addEventListener('DOMContentLoaded', function() {
                document.addEventListener('change', function(event) {
                    if (event.target.matches('.dt-table tbody input[type="checkbox"]')) {
